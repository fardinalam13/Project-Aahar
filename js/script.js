...
// JavaScript code for displaying advice based on BMI
function displayAdvice(bmi) {
	let advice = '';
	if (bmi < 18.5) {
		advice = 'You are underweight. Consider eating a balanced diet and consulting a doctor.';
	} else if (bmi >= 18.5 && bmi < 24.9) {
		advice = 'You have a healthy weight. Keep up the good work!';
	} else if (bmi >= 25 && bmi < 29.9) {
		advice = 'You are overweight. Focus on maintaining a healthy diet and exercising regularly.';
	} else {
		advice = 'You are obese. Consult a healthcare professional for guidance.';
	}
	document.getElementById('result').innerHTML = `<p>Your BMI is ${bmi.toFixed(2)}. ${advice}</p>`;
}
