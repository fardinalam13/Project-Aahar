...
function resend_email_verify($vendor_name,$email,$verify_token)
{
...
if(isset($_POST['vendor_resend_email_verify_btn']))
{
    if(!empty(trim($_POST['email'])))
    {
        $email = mysqli_real_escape_string($con, $_POST['email']);

        $checkemail_query = "SELECT * FROM vendors WHERE email = '$email' LIMIT 1"; 
        ...
        if(mysqli_num_rows($checkemail_query_run) > 0)
        {
            ...
            if($row['verify_status'] == "0")
            {
                ...
                resend_email_verify($vendor_name,$email,$verify_token);
            ...
            else
            {
                ...
                header("Location: vendor_resend-email.php");
                exit(0); 
            }
        }
        else
